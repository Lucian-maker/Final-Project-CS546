		...
		try {
			...
			if (req.query.violationId) {
				res.locals.logDescription = logDescriptions.viewEvidence(
					req.query.violationId,
				);
			} else if (req.query.propertyId) {
				res.locals.logDescription = `Viewed evidence for property ${req.query.propertyId}`;
			} else {
...
router.route("/:id/delete").post(async (req, res) => {
	try {
		...
		res.locals.logCategory = logCategories.evidence;
		res.locals.logDescription = logDescriptions.deleteEvidence(
			req.params.id,
		);
